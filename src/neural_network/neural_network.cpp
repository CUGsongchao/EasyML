...
NeuralNetwork::NeuralNetwork(const std::vector<unsigned int> &sizes) {
    ...
    // This random initialization gives our stochastic
    // gradinet descent algorithm a place to start from.
    for (unsigned i = 1; i < num_layers_; i++){
        // file the bias and weight with normally distributed 
        // random numbers with the mean 0 and standard deviation 1
...
void NeuralNetwork::UpdateMiniBatch(const cv::Mat &subset_data, float eta) {
    ...
    for (int l = 0; l < num_layers_ - 1; l++) {
        weights_[l] *= (1 - eta * lambda / totle_number); // xavier method
...
void NeuralNetwork::BackPropagation(const cv::Mat &x, const cv::Mat &y,
    ...
    // delta = \nabla C_a * g'(z)
    cv::Mat delta = cost_derivation(activations.back(), y, MSE) \
        .mul(sigmoid_primer(weighted_input.back()));
